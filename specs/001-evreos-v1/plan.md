...
display-name change on that account. Both are recorded in the constitution's
Sync Impact Report and in `CLAUDE.md` as issue #26. The `Co-Authored-By` ban and
the session-identifier ban are unenforced mechanically and rest on review.
...
with what satisfies it. (d) A confirmed defect: `measure_download_size()` reads
`target/release/evreos-shell` and `run_gates` keys measurements on `(criterion,
name)` with no platform, so one Linux ELF built on a hosted Linux runner is
compared against **both** the `windows` and the `macos` download-size entries —
and Linux is the deferred platform, so neither entry's stated condition is met
by it. **Plan**: the schema and the fourteen missing entries land before any
...
| **Principle II: the download-size measurement compares one Linux ELF against both platform entries.** `measure_download_size()` reads `target/release/evreos-shell` and `run_gates` keys measurements on `(criterion, name)` with no platform. Linux is the deferred platform, and neither entry's stated condition — "the installer artefact CI publishes" — is met by it. | Recorded rather than silently corrected because it is a confirmed defect in a merged gate and its fix is a change to the gate's measurement key and to CI's artefact production, which belongs with the first installer rather than before it. Until then the figure is a floor, not the entry it is compared against. | Deleting the measurement until an installer exists was rejected: an unmeasured entry now blocks the budget-file gate, and a deferral that is stated (`--allow-unmeasured`, with what satisfies it named in the workflow) is auditable where a silently removed measurement is not. |
