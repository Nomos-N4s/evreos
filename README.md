...
whose key ends `-by` or `-with`, and only in the message's trailing paragraph; plus a
...
the footer strings are matched against the whole text, so an ordinary layout sentence
about a text caret is rejected, and so is a plainly descriptive sentence naming a tool
this project does integrate with — which Principle I's carve-out permits. Phrasings
outside that fixed list pass, as does a trailer placed anywhere but the last paragraph.
