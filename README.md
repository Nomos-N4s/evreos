...
the footer strings are matched against the whole text, so a plainly descriptive sentence
naming a tool this project does integrate with is rejected — which Principle I's carve-out
permits — and so is a layout sentence like "the caret rect is generated by cursor
metrics", since one everyday browser word sits behind that prefix. Phrasings
outside that fixed list pass, and so does a trailer the trailer check does not read — one
outside the last paragraph, or under a key not ending `-by` or `-with` — unless its own
text hits the footer list, which a literal co-authorship line naming a tool does from
