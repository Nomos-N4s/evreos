...
- Operational detail this file does add. None of it grants a permission the
  ...
  - `scripts/check-commit-hygiene.py` checks, on every pull request: author and
    ...
    - The footer strings are matched against the whole text, not just a trailer
      block, so they fire on prose containing the literal sequence `generated
      with` or `generated by` followed by a tool name, the anthropic no-reply
      address, or a literal co-authorship line naming a tool. That rejects a
...
- The first four bullets of this section restate rules the constitution also
  ...
  here for reach. One of them, the commit-message bullet, is gated mechanically
  by the hygiene check. The other three rest on review: `main` carries no branch
  ...
  here and rest on review. When the
